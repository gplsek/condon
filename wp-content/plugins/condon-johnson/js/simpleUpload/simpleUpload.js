...
/* End callback wrappers */
	...
	var num_files = 0;
	  ...
	  if (num_files > 0)
	  {
	    ...
	    var $DOM_file = jQuery(DOM_file);

	    hidden_form = jQuery('<form>').hide().attr("enctype", "multipart/form-data").attr("method", "post").appendTo('body');
	        ...
	        upload: {
	          ...
	          cancel: function(){
	              ...
	              setUploadState(i, 4);
	                ...
	                {
	                jQuery('iframe[name=simpleUpload_iframe_' + private_upload_data[i].iframe + ']').attr("src", "javascript:false;"); //for IE
	...
	function uploadFile(upload_num)
	{
	  ...
	  if (files!=null) //HTML5
	  {
	    ...
	    if (files[upload_num]!=undefined && files[upload_num]!=null)
	    {
	      ...
	      var ajax_xhr = jQuery.ajaxSettings.xhr();
	        ...
	        jQuery.ajax(ajax_settings); //execute ajax request
...
simpleUpload.queueIframe = function(opts){
  ...
  jQuery('body').append('<iframe name="simpleUpload_iframe_' + id + '" style="display: none;"></iframe>');
...
simpleUpload.dequeueIframe = function(id){
    ...
    jQuery('iframe[name=simpleUpload_iframe_' + id + ']').remove(); //remove iframe from the DOM
...
simpleUpload.convertDataType = function(expected_type, declared_type, data){
  ...
  var type = "auto"; //ultimate type to return data as
    ...
    /*
     * Attempt to keep data conversions similar to that of jQuery's jQuery.ajax() function to stay consistent
    ...
    // Output: object, null
      ...
      if (typeof data=="string")
      {
        ...
        try {
          ...
          return jQuery.parseJSON(data); //JSON is valid, return object or null
    ...
    // Output: object (XMLDoc), null
      ...
      if (typeof data=="string")
      {
        ...
        try {
          ...
          return jQuery.parseXML(data); //XML is valid, return object (native XMLDocument object) or null
    ...
    // Output: string
      ...
      if (typeof data=="string")
      {
        ...
        try {
          ...
          jQuery.globalEval(data); //execute script in the global scope

          return data; //return script as string

        } catch(e) {

          return false; //there was an error while executing the script with jQuery.globalEval() - the script still may have run partially, but this is consistent behavior with jQuery.ajax()
...
simpleUpload.postMessageCallback = function(e){
    ...
    try {
      ...
      var data = e[key];
        ...
        data = jQuery.parseJSON(data); //convert JSON string to object (throws error on malformed JSON for jQuery >= 1.9, can return null if data is empty string, null, or undefined for older versions of jQuery)
...
(function (factory) {
  ...
  jQuery.fn.simpleUpload = function(url, opts){

    //if calling with jQuery.fn.simpleUpload() and the "files" option is present, go ahead and pass it along...
  ...
  jQuery.fn.simpleUpload.maxSimultaneousUploads = function(num){
